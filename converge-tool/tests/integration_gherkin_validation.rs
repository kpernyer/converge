...
static SHARED_PROVIDER: LazyLock<Arc<dyn LlmProvider>> = LazyLock::new(|| {
    ...
    let api_key = std::env::var("ANTHROPIC_API_KEY").unwrap_or_else(|_| {
        ...
        panic!(
            "ANTHROPIC_API_KEY environment variable not set. Test cannot proceed without API key."
        );
    });
    ...
    AnthropicProvider::from_env(model)
        ...
        .unwrap_or_else(|e| {
            ...
            panic!("Failed to create Anthropic provider: {e}. Test cannot proceed.");
        })
});
...
";
    ...
    println!(
        "   API calls made: {} (business sense + compilability checks)",
    ...
    assert!(
        !result.has_errors(),
        "Should not have errors for valid spec"
    );
}
...
";
    ...
    assert!(
        ...
        "Should detect business sense issues in untestable scenario. Result: {result:?}"
    );
}
...
";
    ...
    assert!(
        ...
        "Should detect vague preconditions. Result: {result:?}"
    );
}
...
";
    ...
    let has_uncertain_issue = result.issues.iter().any(|i| {
        i.category == IssueCategory::Convention
...
";
    ...
    let has_name_issue = result
        .issues
        .iter()
        .any(|i| i.category == IssueCategory::Convention && i.message.contains("name"));
...
";
    ...
    if !result.issues.is_empty() {
        ...
        for issue in &result.issues {
            if let Some((_, issues)) = by_category
                .iter_mut()
